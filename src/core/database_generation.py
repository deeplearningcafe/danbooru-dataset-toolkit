...
def load_completed_tags(log_path: str) -> set:
    """
    Loads already downloaded tags from a log file to avoid re-downloading.
...
def fetch_page_with_retry(session, tag, page):
    """Fetches a single page of results for a given tag with retries."""
    ...
    cookies = {'cf_clearance': "YOUR-COOKIE",}
...
def get_newest_ids_for_tags(
    ...
    """
    Reads the existing dataset to find the newest post ID for each tag.

    This is used to prevent re-downloading posts that are already present
    when updating tags with new content.

    Args:
        dataset_path (str): The path to the existing dataset file.
        tags (Set[str]): A set of tags for which to find the newest IDs.

    Returns:
        Dict[str, int]: A dictionary mapping each tag to its newest post ID.
    """
    ...
    all_artist_tags_in_df = set(
        ' '.join(df['tag_string_artist']).split()
    )
    ...
    df['tag_count_character'] = df['tag_string_character'].str.split(' ').apply(
    ...
    if artist_tags_to_keep:
        ...
        artist_regex = '|'.join(
            fr'(?:^|\s){re.escape(tag)}(?:$|\s)'
            for tag in artist_tags_to_keep
        )
        df['has_priority_artist'] = df[
            'tag_string_artist'
        ].str.contains(artist_regex, na=False)
    else:
    ...
    for tag in character_tags_to_process:
        ...
        regex_pattern = fr'(?:^|\s){re.escape(tag)}(?:$|\s)'

        mask = df['tag_string_character'].str.contains(
        ...
        if mask.any():
            ...
            removal_mask = (
                mask &
                (df['tag_count_character'] == 1) &
                (~df['has_priority_artist'])
            )
    ...
    for tag in artist_tags_to_keep:
        ...
        if mask.any():
            ...
            newest_ids[tag] = max(
                newest_ids.get(tag, 0), df.loc[mask, 'id'].max()
            )
...
def fetch_all_for_tag(session, tag, stop_at_id: int = None):
    """
    Fetches all posts for a single tag by paginating.

    Stops fetching if a post ID is encountered that is less than or equal
    to `stop_at_id`, allowing for incremental updates of existing tags.
    """
    all_posts = []
    page = 1
    while True:
        ...
        if stop_at_id is not None:
            # from np.int64 to python int
            stop_at_id = int(stop_at_id)
            # Posts are ordered by ID descending. Check the last post on the
            # page to see if we've reached posts we already have.
            last_post_id = posts[-1].get('id')
            if last_post_id and last_post_id <= stop_at_id:
                # Filter this page for only the posts newer than our newest
                # existing post, then stop fetching more pages.
                new_posts = [p for p in posts if int(p.get('id')) > stop_at_id]
...
def create_prior_knowledge_dataset(
    ...
    completed_tags_without_id = [tag for tag in completed_tags if newest_ids_map.get(tag) is None]
    ...
    with requests.Session() as session:
        for i in range(0, len(tags_to_process), batch_size):
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                ...
                for tag in batch_tags:
                    ...
                    future = executor.submit(
                        fetch_all_for_tag, session, tag, stop_at_id=stop_id
                    )
                ...
                for future in as_completed(future_to_tag):
                    ...
                    try:
                        ...
                        if result:
                            ...
                            print(f"Successfully fetched {len(result)} posts for tag: '{tag}'")
                    except Exception as e:
            ...
            columns_to_keep = [
                'id', 'created_at', 'score', 'source', 'md5', 'rating',
                'image_width', 'image_height', 'tag_string', 'fav_count',
                'file_ext', 'parent_id', 'is_deleted', 'is_banned',
                'file_url', 'large_file_url', 'preview_file_url',
                'tag_string_general', 'tag_string_character',
                'tag_string_copyright', 'tag_string_artist',
                'tag_count_character', 'tag_count_copyright', 'tag_string_meta'
            ]
            ...
            try:
                ...
                df_batch.to_csv(
                    output_csv_path, mode='a', index=False, header=header
                )
                print(
                    f"Saved {len(df_batch)} new unique posts to '{output_csv_path}'"
                )

                # Log only the newly completed tags to allow resuming.
                # Updated tags are already in the completed log.
                newly_completed_tags = [
                    tag for tag in successfully_downloaded_tags
                    if tag in tags_to_fetch
                ]
                if newly_completed_tags:
                    with open(
                        completed_tags_log_path, 'a', encoding='utf-8'
                    ) as f:
