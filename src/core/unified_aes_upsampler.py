...
def unified_aes_upsampler(
    ...
    print(f"Searching data in {root_dir}, with aesthetic_labels_csv {aesthetic_labels_csv} and upsampled_tags_path {upsampled_tags_path}")
    # 1. Load Models
    print("Loading models for unified processing...")
    # Load the SwinV2 model which serves as both tagger and feature extractor
    tagger = Tagger(
        model_dir=tagger_model_dir,
        device=device,
    ...
    ckpt = torch.load(
        classifier_path, map_location=device
    )
    ...
    pd.DataFrame(classification_results).to_csv(
        aesthetic_labels_csv, index=False
    )
