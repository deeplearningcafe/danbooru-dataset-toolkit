...
from ..utils.loader import load_all_parquets, parallel_scan_images, append_weight_to_json
...
class PromptGenerator:
    ...
    def load_booru_df(
        ...
        full_df = load_all_parquets(parquet_path, True if prior_knowledge_path else False, num_parquets=num_parquets)
        columns_to_keep = [
            'id', 'created_at', 'relative_path', 'tag_string_general',
            'tag_string_character', 'tag_string_copyright', 'rating',
            'tag_string_artist', 'final_tier', 'quality_tier',
            'image_width', 'image_height',  'tag_string_meta'
        ]
        existing_columns = [
            col for col in columns_to_keep if col in full_df.columns
        ]
        full_df = full_df[existing_columns]

        if prior_knowledge_path:
            prior_knowledge_samples = pd.read_csv(prior_knowledge_path, header=0, low_memory=True)
            ...
            full_df = pd.concat([full_df, prior_knowledge_samples]).drop_duplicates(subset=['id'], keep='last').reset_index(drop=True)
        ...
        for cls_path in classifier_labels:
            if cls_path.endswith('.csv'):
                ...
                classifier_labels_df = pd.DataFrame(list(json_data.items()), columns=['relative_path', 'aesthetic_label'])
                label_to_class = {0: "worse_score", 1: "bad_score", 2: "good_score", 3: "masterpiece"}
                # idx2label = {0:"worst", 1:"worse", 2:"better", 3:"best"} classifier format
                classifier_labels_df['aesthetic_label'] = classifier_labels_df['aesthetic_label'].apply(
                    lambda label: label_to_class[int(label)]
                )
                classifier_labels_df_list.append(classifier_labels_df)
            else:
                raise ValueError(f"Unsupported file format: {classifier_labels}. Only CSV and JSON are supported.")
        ...
        classifier_labels_df['id'] = classifier_labels_df['relative_path'].apply(
            lambda p: int(os.path.splitext(os.path.basename(p.replace('\\', os.sep)))[0])
        )
        ...
        if sampled_ids_csv is not None:
            ...
            # )
            classifier_labels_df['quality_tier'] = classifier_labels_df['new_aesthetic_label']

            processed_df = classifier_labels_df
        print(f"Len processed_df {len(processed_df)}")

        columns_to_include = ['id', 'quality_tier', 'new_aesthetic_label', 'relative_path']
        if upsampled_prompts_csv is not None and len(upsampled_prompts_csv)>0:
            ...
            upsampled_prompts_df["upsampled_tags"] = upsampled_prompts_df["upsampled_tags"].fillna('')
        ...
        final_df = pd.merge(
            full_df, processed_df[columns_to_include],
            on='id', how='inner'
        )
    ...
    def load_booru_df_sampled(
        ...
        if prior_knowledge_path:
            prior_knowledge_samples = pd.read_csv(prior_knowledge_path, header=0, low_memory=False)
            existing_columns = [
                col for col in columns_to_keep if col in prior_knowledge_samples.columns
            ]
            prior_knowledge_samples = prior_knowledge_samples[existing_columns]
    ...
    def refine_tiers_and_assign_final_class(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'quality_tier' not in df.columns or \
        'new_aesthetic_label' not in df.columns:
            raise ValueError("Input DataFrame must contain 'quality_tier' and "
                            "'new_aesthetic_label' columns.")
        ...
        demotion_mask = (
            df['quality_tier'].isin(['masterpiece', 'good_score', 'bad_score']) &
        ...
        demotion_mask = (
            df['quality_tier'].isin(['masterpiece', ]) &
        ...
        promotion_mask = (
            df['quality_tier'].isin(['bad_score', 'good_score']) &
        ...
        promotion_mask = (
            df['quality_tier'].isin(['worse_score']) &
        ...
        try:
            ...
            print(f"Error: Could not write to file {self.config['final_tiers_csv']}. Reason: {e}")
    ...
    def prune_and_report_tags(
        ...
        allowed_characters = set(
            tags_df[tags_df['category'] == 4]['name']
        )
        ...
        normalized_upsampled_tags = df['upsampled_tags'].apply(
            normalize_tag_string
        )
        ...
        full_report[['category', 'tag', 'count']].to_csv(
            'tag_counts_report.csv', index=False
        )
        ...
        remove_general = set(
            general_counts[general_counts < min_general_count].index
        )
        ...
        remove_artist = {
            tag for tag, count in artist_counts.items()
        ...
        upsampled_remove_set = {
            format_danbooru_tag(tag) for tag in remove_general
        }
        ...
        if 'upsampled_tags' in df.columns and upsampled_regex:
            ...
            df['upsampled_tags'] = df['upsampled_tags'].str.replace(
                upsampled_regex, '', regex=True
            ).str.replace(
                r',\s*,', ',', regex=True  # Clean up empty tags
            ).str.replace(
                r'(^,\s*|\s*,$)', '', regex=True # Clean leading/trailing commas
            ).str.strip()
    ...
    def count_and_weight_tags(
        ...
        try:
            ...
            valid_booru_ids = {str(item['booru_id']) for item in metadata.get("sample_mapping")}
        ...
        report_path = "tag_counts_report.csv"
        count_tags(
            df,
        ...
        for booru_id in tqdm(valid_txt_paths, desc="Appending weights"):
            ...
            try:
                ...
                print(f"Warning: Failed to update {json_path} with txt file {txt_path}. Reason: {e}")
    ...
    def construct_prompt_string(self, row: pd.Series) -> str:
        ...
        if pd.notna(meta_tags_raw):
            ...
            present_meta_tags = [
                tag for tag in META_TAGS_TO_INCLUDE
        ...
        if free_tokens > 0 and len(upsampled_tags_raw)>0:
            decoded_sliced_input_ids, last_token = split_upsampled_tags(
                upsampled_tags_raw, free_tokens, self.tokenizer,
        ...
        general_tokens = self.tokenizer(
                ...
                padding=False, # Don't pad here, just count tokens
                truncation=False, # Don't truncate as we want length
                return_length=True,)["input_ids"][1:-1]

        # 4. Assemble the final prompt using the defined separator
        all_parts = [prompt_prefix_str, general_tags, decoded_sliced_input_ids,
                    prompt_suffix_str]
        all_parts = [prompt for prompt in all_parts if len(prompt) > 0]
        final_prompt = ", ".join(all_parts)

        return final_prompt, [prompt_prefix_tokens, general_tokens, prompt_suffix_tokens]
    ...
    def create_prompt_files(
        ...
        for _, row in tqdm(df.iterrows(), total=df.shape[0],
                        desc="Writing prompt files"):
            prompt_text, tokens = self.construct_prompt_string(row)
            relative_path = os.path.splitext(row['relative_path'].replace('\\', os.sep))[0]
            exists = False
            ...
            if not exists:
                print(f"The image with relative path {row['relative_path']} doesn't exists")
