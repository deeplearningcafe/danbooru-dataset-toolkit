...
from ..prompts.prompt_utils import format_danbooru_tag_inverse, count_tags, analyze_tag_distribution

def load_and_merge_classifier_labels(
    df: pd.DataFrame,
    classifier_csv_path: str,
    verbose: bool = True
) -> pd.DataFrame:
    """
    ...
    if not os.path.exists(classifier_csv_path):
        if verbose:
            print(f"Warning: Classifier labels file not found at "
...
def create_quality_tiers(
    ...
    required_core_cols = [
        "id", "score", "fav_count", "aes_score", "tag_string"
    ]
    missing_core_cols = [
        col for col in required_core_cols if col not in df.columns
    ]
    if missing_core_cols:
        raise ValueError(
            f"Core columns {missing_core_cols} not found in DataFrame."
        )
    ...
    if 'aes_score' in df_processed.columns:
        ...
        if verbose:
            ...
            if removed_na_count > 0:
                print(f"Removed {removed_na_count} rows with NaN 'aes_score' "
    ...
    if "is_deleted" in df_processed.columns:
        df_processed['is_deleted'] = df_processed['is_deleted'].astype(
            'boolean'
        )
        count_before = len(df_processed)
        df_processed = df_processed[
            df_processed["is_deleted"].fillna(False) == False
        ]
        if verbose:
            print(f"Removed {count_before - len(df_processed)} 'is_deleted' "
                  f"items. Size: {len(df_processed)}")

    if "is_banned" in df_processed.columns:
        df_processed['is_banned'] = df_processed['is_banned'].astype(
            'boolean'
        )
        count_before = len(df_processed)
        df_processed = df_processed[
            df_processed["is_banned"].fillna(False) == False
        ]
        if verbose:
            print(f"Removed {count_before - len(df_processed)} 'is_banned' "
                  f"items. Size: {len(df_processed)}")
    ...
    if "md5" in df_processed.columns:
        ...
        if verbose:
            print(f"Removed {count_before - len(df_processed)} MD5 duplicates "
                  f"(kept highest score). Size: {len(df_processed)}")

    # Filter out low-resolution images
    if ("image_width" in df_processed.columns and
            "image_height" in df_processed.columns):
        count_before = len(df_processed)
        # Calculate the total number of pixels for each image.
        # The calculation is done element-wise for efficiency.
        image_pixels = (
                df_processed["image_width"] * df_processed["image_height"]
        )
    ...
    if "parent_id" in df_processed.columns and "id" in df_processed.columns:
        ...
        df_processed['tag_count'] = df_processed[
            'tag_string'
        ].str.split(' ').str.len()
        ...
        parents_in_df = df_processed[
            df_processed['id'].isin(unique_parent_ids)
        ]
        ...
        for parent_id in unique_parent_ids:
            ...
            current_children = children_df[
                children_df['parent_id'] == parent_id
            ]
            ...
            if num_children == 1:
                ...
                ids_to_keep_from_children = {
                    child_max_tags['id'], child_max_score['id']
                }
        ...
        if verbose:
            removed_count = count_before - len(df_processed)
            print(f"Processed parent/child relationships, removing "
    ...
    if negative_tags_list is None:
        negative_tags_list = [
            "bad_anatomy", "bad_hands", "bad_feet", "bad_perspective", "text",
            "bad_proportions", "extra_digits", "lowres", "jpeg_artifacts",
            "blurry", "low_quality", "worst_quality", "gif_artifacts", "corrupted_file",
            "watermark", "banned_artist", "error", "cropped", "bad_aspect_ratio",
            "scan_artifacts", "scan_dust", "adversarial_noise"
        ]
    if quality_tags_list is None:
        quality_tags_list = ["highres", "absurdres", "ultra-detailed", "official_art",
            "novel_illustration", "official_wallpaper", "incredibly_absurdres",
            "key_visual", "promotional_art"]
    ...
    if 'tag_count_character' not in df_processed.columns:
        ...
        df_processed['tag_count_character'] = df_processed[
            'tag_string_character'
        ].fillna('').str.split(' ').apply(
            lambda tags: len(tags) if tags != [''] else 0
        )
    ...
    if 'tag_count_copyright' not in df_processed.columns:
        ...
        df_processed['tag_count_copyright'] = df_processed[
    ...
    if 'tag_count_character' in df_processed.columns:
        ...
        df_processed = df_processed[df_processed['tag_count_character'] <= max_num_chars]
        df_processed = df_processed[df_processed['tag_count_copyright'] <= max_num_chars]
    ...
    if verbose:
        ...
        print(f"Proceeding with quality tiering on {len(df_processed)} "
              "images.")
    ...
    count_tags(
        ...
        output_path="tag_counts_sampling_filtered.csv")

    df_processed['negative_tag_count'] = 0
    for tag in negative_tags_list:
        df_processed['negative_tag_count'] += df_processed[
            'tag_string_lower'
        ].str.contains(tag.lower(), regex=False).astype(int)

    df_processed['positive_tag_count'] = 0
    for tag in quality_tags_list:
        df_processed['positive_tag_count'] += df_processed[
            'tag_string_lower'
        ].str.contains(tag.lower(), regex=False).astype(int)
    ...
    s_p20, s_p60, s_p92 = np.percentile(
        df_processed['score'].values, [20, 60, 92]
    ) if len(df_processed['score'].unique()) > 3 else (0,0,0)

    f_p20, f_p60, f_p92 = np.percentile(
        df_processed['fav_count'].values, [20, 60, 92]
    ) if len(df_processed['fav_count'].unique()) > 3 else (0,0,0)

    aes_p25, aes_p38, aes_p50 = np.percentile(
        df_processed['aes_score'].values, [25, 38, 50]
    ) if len(df_processed['aes_score'].unique()) > 4 else (0,0,0,0)
    if verbose:
    ...
    if verbose:
        ...
        print(len(df_processed.loc[((df_processed['score'] >= s_p92) &(df_processed['fav_count'] >= f_p92)) ]))
    ...
    df_processed.loc[worse_score_mask_tags | worse_score_mask_metrics, 'quality_label'] = "worse_score"
    ...
    if verbose:
        ...
        print(len(df_processed.loc[((df_processed['quality_label'] == "unassigned") &
        (df_processed['score'] >= s_p60)) ]))
    ...
    df_processed.loc[df_processed['quality_label'] == "unassigned", 'quality_label'] = "bad_score"
    ...
    if 'new_aesthetic_label' in df_processed.columns:
        ...
        demotion_mask = (
            df_processed['quality_label'].isin(['masterpiece', 'good_score']) &
        ...
        if num_demoted > 0:
            ...
            if verbose:
                print(f"Demoted {num_demoted} images from high-quality "
        ...
        promotion_mask = (
            df_processed['quality_label'].isin(['bad_score', 'worse_score']) &
        ...
        if num_promoted > 0:
            ...
            if verbose:
                print(f"Promoted {num_promoted} images from low-quality "
                      "tiers to 'good_score' based on new classifier.")


    # Create the four tier DataFrames
    masterpiece_df = df_processed[
        df_processed['quality_label'] == "masterpiece"
    ].copy()
    good_score_df = df_processed[
        df_processed['quality_label'] == "good_score"
    ].copy()
    bad_score_df = df_processed[
        df_processed['quality_label'] == "bad_score"
    ].copy()
    worse_score_df = df_processed[
    ...
    if verbose:
        ...
        for name, T_df in tier_info.items():
            ...
            if not T_df.empty:
                print(f"  Score range: {T_df['score'].min():.1f} - "
                      f"{T_df['score'].max():.1f}, "
                      f"Mean: {T_df['score'].mean():.1f}")
                print(f"  Fav range: {T_df['fav_count'].min():.1f} - "
                      f"{T_df['fav_count'].max():.1f}, "
                      f"Mean: {T_df['fav_count'].mean():.1f}")
                print(f"  Aes_score range: {T_df['aes_score'].min():.2f} - "
                      f"{T_df['aes_score'].max():.2f}, "
                      f"Mean: {T_df['aes_score'].mean():.2f}")
                print(f"  Avg Negative Tags: "
                      f"{T_df['negative_tag_count'].mean():.2f}")
                if 'rating' in T_df.columns: # If rating column exists
                    dist_str = ", ".join([
                        f"{idx}: {val:.1%}" for idx, val in
                        T_df['rating'].value_counts(normalize=True).items()
                    ])
...
def _sample_from_tier(
    ...
    if include_tags:
        ...
        available_tag_columns = [
            col for col in tag_columns if col in work_df.columns
        ]
        ...
        work_df['sampling_weight'] = work_df.apply(
            _calculate_weight_for_row, axis=1
        )
    else:
    ...
    for i in range(remainder):
        rating_to_increment = list(rating_targets.keys())[
            i % len(rating_targets)
        ]
    ...
    print(f"The ratings distribution didn't fill the tier target count, sampling {remaining_needed} samples from all ratings.")
    ...
    *This function no longer handles image downloading.*
    ...
    if prior_knowledge_path and os.path.exists(prior_knowledge_path):
        ...
        try:
            ...
            if 'fav_count' in prior_knowledge_samples.columns:
                ...
                print("  - Warning: aes_scores_csv_path not provided. 'aes_score' may be missing.")

        except Exception as e:
            print(f"  - Warning: Could not load or process prior knowledge file: {e}")
            prior_knowledge_samples = pd.DataFrame()

    # If a character list is provided, we filter BOTH the prior knowledge
    # and the main dataframe immediately. This ensures all subsequent
    # tiering logic applies only to the desired characters.
    single_characters = False
    if len(character_list) > 0:
        single_characters = True
        if verbose:
            print(f"\n--- Filtering for specific characters: {character_list} ---")

        # Create a regex pattern to match tags exactly (whole words)
        # Pattern matches: (Start OR Space) + Tag + (End OR Space)
        # This prevents "anna" from matching "annabelle"
        ...
        if not prior_knowledge_samples.empty:
            if 'tag_string_character' in prior_knowledge_samples.columns:
                prior_mask = prior_knowledge_samples['tag_string_character'].str.contains(
                    char_pattern, regex=True, na=False
                )
                before_len = len(prior_knowledge_samples)
                prior_knowledge_samples = prior_knowledge_samples[prior_mask]
                if verbose:
                    print(f"  - Prior Knowledge filtered from {before_len} to "
                          f"{len(prior_knowledge_samples)} samples.")
            else:
                if verbose:
                    print("  - Warning: 'tag_string_character' missing in prior "
                          "knowledge. Skipping character filter for prior.")

        # Filter Main DataFrame
        if 'tag_string_character' in df.columns:
            main_mask = df['tag_string_character'].str.contains(
                char_pattern, regex=True, na=False
            )
            before_len = len(df)
            df = df[main_mask]
            if verbose:
                print(f"  - Main Dataset filtered from {before_len} to "
                      f"{len(df)} samples.")
        else:
            print("  - Warning: 'tag_string_character' missing in main dataframe. "
                  "Cannot filter for characters.")
            return pd.DataFrame(), {}

        if df.empty and prior_knowledge_samples.empty:
            print("  - Error: No samples found for the requested characters.")
            return pd.DataFrame(), {}

    # Exclude prior knowledge and already-excluded IDs from the main df
    prior_ids = set(prior_knowledge_samples['id']) if not prior_knowledge_samples.empty else set()
    exclude_df = None
    ...
    if not prior_knowledge_samples.empty:
        if verbose:
            ...
            count_tags(prior_knowledge_samples, output_path="tag_counts_prior_knowledge.csv")
        ...
        masterpiece_prior, good_prior, bad_prior, worse_prior = create_quality_tiers(
            prior_knowledge_samples, single_characters=single_characters, verbose=verbose
        )

        # only generate tags counts for each tier if verbose
        if verbose:
            files_to_process = [
                "tag_counts_masterpiece.csv",
                "tag_counts_good.csv",
                "tag_counts_bad.csv",
                "tag_counts_worse.csv",
            ]

            for name, df in zip(files_to_process, [masterpiece_prior, good_prior, bad_prior, worse_prior]):
            ...
            if not final_distribution_df.empty:
                output_filename = "merged_tag_counts.csv"
        ...
        if verbose:
            print("  - Splitting prior knowledge tiers into Artists and "
                  "Characters subsets...")
        ...
        if artist_tags and 'tag_string_artist' in masterpiece_prior.columns:
            ...
            is_artist_mask_mp = masterpiece_prior['tag_string_artist'].isin(
                artist_tags
            )
            masterpiece_prior_artists = masterpiece_prior[is_artist_mask_mp].copy()
            masterpiece_prior_artists['quality_tier'] = masterpiece_prior_artists['quality_label']
            masterpiece_prior_chars = masterpiece_prior[~is_artist_mask_mp].copy()
            masterpiece_prior_chars['quality_tier'] = masterpiece_prior_chars['quality_label']

            # 2. Split the Good Score Tier
            is_artist_mask_good = good_prior['tag_string_artist'].isin(
                artist_tags
            )
            ...
            is_artist_mask_bad = bad_prior['tag_string_artist'].isin(
                artist_tags
            )
            ...
            is_artist_mask_worse = worse_prior['tag_string_artist'].isin(
                artist_tags
            )
            ...
            bad_prior_artists = pd.concat([bad_prior_artists, worse_prior_artists], ignore_index=True)
            bad_prior_artists['quality_tier'] = bad_prior_artists['quality_label']

            if verbose:
                print(f"  - Upgraded {num_to_upgrade} artist samples "
                        "from 'worse_score' to 'bad_score'.")

            # Apply skip_tags logic with different probabilities ---
            if skip_tags:
                if verbose:
                    print("  - Applying skip_tags to prior knowledge "
                          "subsets...")
                ...
                masterpiece_prior_artists = filtered_artist_dfs[
                    "Masterpiece Artists"
                ]
                good_prior_artists = filtered_artist_dfs[
                    "Good Score Artists"
                ]
                bad_prior_artists = filtered_artist_dfs[
                    "Bad Score Artists"
                ]

            if verbose:
                # Optional: Print stats about the split for verification
                mp_a, mp_c = len(masterpiece_prior_artists), len(masterpiece_prior_chars)
            ...
            if verbose:
                print("  - Warning: No artist tags found or 'tag_string_artist' "
        ...
        all_sampled_dfs.extend([masterpiece_prior_artists, good_prior_artists, bad_prior_artists])
        if verbose:
        ...
        already_sampled_tiers = {
            'masterpiece': 0, 'good_score': 0, 'bad_score': 0, 'worse_score': 0
        }
    ...
    if num_prior_samples >= total_samples:
        if verbose:
            print(f"Prior knowledge count ({num_prior_samples}) meets target {total_samples}. Sampling only from it.")
        ...
        final_df[['id', 'quality_tier', 'rating', 'tag_string', 'file_url', 'large_file_url', 'image_width', 'image_height']].to_csv(output_csv, index=False)
    ...
    if non_prior_samples > 0:
        masterpiece_df, good_df, bad_df, worse_df = create_quality_tiers(
            df, single_characters=single_characters, verbose=verbose
        )
        ...
        all_sampled_dfs.extend([masterpiece_prior_chars, good_prior_chars, bad_prior_chars, worse_prior_chars])

        num_prior_samples = sum(already_sampled_tiers.values())
        if verbose:
            print(f"Included character tags as already sampled. {num_prior_samples} already sampled.")
    ...
    tier_targets = {
        name: int(total_samples * perc)
        for name, perc in quality_percentages.items()
    }
    ...
    for tier_name in tier_order:
        ...
        if tier_df is None or tier_df.empty or current_target <= 0:
            stats[tier_name] = {'available': len(tier_df) if tier_df is not None else 0, 'target': current_target, 'sampled': 0}
        ...
        if not tier_samples.empty:
            ...
            if verbose:
                print(f"{tier_name}  df repeated ids {sum(tier_samples['id'].value_counts() > 1)}")
    ...
    final_df = pd.concat(all_sampled_dfs, ignore_index=True)
    if verbose:
        print(f"Final df quality_tier nans {sum(final_df['quality_tier'].isna()), sum(final_df['quality_tier'].isnull())}")
        print(f"Final df quality_label nans {sum(final_df['quality_label'].isna()), sum(final_df['quality_label'].isnull())}")
    ...
    if remaining_needed > 0:
        if verbose:
            print(f"  - Main sampling got {len(final_df)}. "
        ...
        if leftover_dfs:
            ...
            if not leftover_pool.empty:
                ...
                fill_samples = leftover_pool.sample(
                    n=min(remaining_needed, len(leftover_pool)),
                    random_state=rng
                )
    ...
    if verbose:
        ...
        for name, s in stats.items():
            ...
            print(f"  {name}: Sampled {total_s} "
                  f"(from {s['available']} available)")

    count_tags(
        final_df,
        normalized_upsampled_tags=None,
        output_path="tag_counts_sampled_ids.csv"
    )
